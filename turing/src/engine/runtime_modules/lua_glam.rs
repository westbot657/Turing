...
use mlua::{FromLua, FromLuaMulti, IntoLua, IntoLuaMulti, Lua, MaybeSend, Table, UserData, UserDataMethods, Value};
    ...
    ( $methods:expr, $lua_ty:path) => {
        {
            ...
            $methods.add_method("dot", |_, this, rhs: $lua_ty| {
                Ok(this.0.dot(rhs.0))
            });
            ...
            $methods.add_method("copy", |_, this, _: ()| {
                Ok($lua_ty(this.0))
            });
        }
    };
...
impl UserData for LuaVec2 {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method("extend", |_, this, z: f32| {
            Ok(LuaVec3(this.0.extend(z)))
        });
...
impl UserData for LuaVec4 {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method("truncate", |_, this, _: ()| {
            Ok(LuaVec3(this.0.truncate()))
        })
...
fn create_vec3_tables(lua: &Lua, api: &Table) -> Result<()> {
    ...
    class.create_function(
        lua, "splat",
        |_, v: f32| {
            Ok(LuaVec3(Vec3::splat(v)))
        }
    )?;

    class.create_function(
        lua, "from_homogeneous",
        |_, v4: LuaVec4| {
            if v4.0.w == 0. {
                Err(mlua::Error::runtime("Cannot Create Vec3 from Vec4 with w == 0"))
            } else {
...
fn create_quat_tables(lua: &Lua, api: &Table) -> Result<()> {
    ...
    fun!("from_vec4", |_, v4: LuaVec4| {
        ok!(Quat::from_vec4(v4.0))
    });
    ...
    fun!("from_euler", |_, (euler, x, y, z): (String, f32, f32, f32)| {
        ...
        let euler = match euler.to_uppercase().as_str() {
            ...
            "ZXY" => EulerRot::ZXY,
            "ZYX" => EulerRot::ZYX,
            _ => return Err(mlua::Error::runtime(format!("Invalid euler ordering: {}", euler)))
        };
    ...
    fun!("look_at_lh", |_, (eye, center, up): (LuaVec3, LuaVec3, LuaVec3)| {
        ok!(Quat::look_at_lh(eye.0, center.0, up.0))
    });

    fun!("look_at_rh", |_, (eye, center, up): (LuaVec3, LuaVec3, LuaVec3)| {
...
fn create_mat4_tables(lua: &Lua, api: &Table) -> Result<()> {
    ...
    fun!("from_cols", |_, (c0, c1, c2, c3): (LuaVec4, LuaVec4, LuaVec4, LuaVec4)| {
    ...
    fun!("from_scale_rotation_translation", |_, (scale, rot, trans): (LuaVec3, LuaQuat, LuaVec3)| {
        ok!(Mat4::from_scale_rotation_translation(scale.0, rot.0, trans.0))
    });
    ...
    fun!("from_quat", |_, q: LuaQuat| {
        ok!(Mat4::from_quat(q.0))
    });
    ...
    fun!("look_to_lh", |_, (eye, dir, up): (LuaVec3, LuaVec3, LuaVec3)| {
        ok!(Mat4::look_to_lh(eye.0, dir.0, up.0))
    });

    fun!("look_to_rh", |_, (eye, dir, up): (LuaVec3, LuaVec3, LuaVec3)| {
        ok!(Mat4::look_to_rh(eye.0, dir.0, up.0))
    });

    fun!("perspective_lh", |_, (fov_y, aspect, near, far): (f32, f32, f32, f32)| {
        ok!(Mat4::perspective_lh(fov_y, aspect, near, far))
    });

    fun!("perspective_rh", |_, (fov_y, aspect, near, far): (f32, f32, f32, f32)| {
        ok!(Mat4::perspective_rh(fov_y, aspect, near, far))
    });

    fun!("perspective_infinite_lh", |_, (fov_y, aspect, near): (f32, f32, f32)| {
        ok!(Mat4::perspective_infinite_lh(fov_y, aspect, near))
    });

    fun!("perspective_infinite_rh", |_, (fov_y, aspect, near): (f32, f32, f32)| {
        ok!(Mat4::perspective_infinite_rh(fov_y, aspect, near))
    });

    fun!("perspective_infinite_reverse_lh", |_, (fov_y, aspect, near): (f32, f32, f32)| {
        ok!(Mat4::perspective_infinite_reverse_lh(fov_y, aspect, near))
    });

    fun!("perspective_reverse_lh", |_, (fov_y, aspect, near): (f32, f32, f32)| {
        ok!(Mat4::perspective_infinite_reverse_rh(fov_y, aspect, near))
    });

    fun!("orthographic_lh", |_, (left, right, bottom, top, near, far): (f32, f32, f32, f32, f32, f32)| {
        ok!(Mat4::orthographic_lh(left, right, bottom, top, near, far))
    });

    fun!("orthographic_rh", |_, (left, right, bottom, top, near, far): (f32, f32, f32, f32, f32, f32)| {
